"""
Binance 5m candle ingestion: load instruments, fetch 5m klines, upsert into market_data_bars_5m.
...
from services.market_data.binance_client import fetch_klines_5m
...
def run_one_cycle(session: Session, limit_per_symbol: int = DEFAULT_KLINES_LIMIT) -> Tuple[int, int, List[str]]:
    """
    Fetch recent 5m klines for each Binance instrument and upsert into market_data_bars_5m.
    ...
    for instrument_id, provider_symbol in instruments:
        try:
            candles = fetch_klines_5m(provider_symbol, limit=limit_per_symbol)
            if not candles:
            ...
            failures.append(msg)
            logger.exception("5m candle ingestion failed for %s", provider_symbol)
