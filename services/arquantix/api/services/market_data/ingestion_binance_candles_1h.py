"""
Binance 1h candle ingestion: load instruments, fetch 1h klines, upsert into market_data_bars_1h.
...
from services.market_data.binance_client import fetch_klines_1h
...
def run_one_cycle(session: Session, limit_per_symbol: int = DEFAULT_KLINES_LIMIT) -> Tuple[int, int, List[str]]:
    """
    Fetch recent 1h klines for each Binance instrument and upsert into market_data_bars_1h.
    ...
    for instrument_id, provider_symbol in instruments:
        try:
            candles = fetch_klines_1h(provider_symbol, limit=limit_per_symbol)
            if not candles:
            ...
            failures.append(msg)
            logger.exception("1h candle ingestion failed for %s", provider_symbol)
