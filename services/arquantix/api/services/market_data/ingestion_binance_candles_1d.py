"""
Binance 1d candle ingestion: load instruments, fetch 1d klines, upsert into market_data_bars_1d.
...
from services.market_data.binance_client import fetch_klines_1d
...
def run_one_cycle(session: Session, limit_per_symbol: int = DEFAULT_KLINES_LIMIT) -> Tuple[int, int, List[str]]:
    """
    Fetch recent 1d klines for each Binance instrument and upsert into market_data_bars_1d.
    ...
    for instrument_id, provider_symbol in instruments:
        try:
            candles = fetch_klines_1d(provider_symbol, limit=limit_per_symbol)
            if not candles:
            ...
            failures.append(msg)
            logger.exception("1d candle ingestion failed for %s", provider_symbol)
