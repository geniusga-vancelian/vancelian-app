"""Service layer for Risk Policies module (Portfolio Engine)."""
...
from .repository import RiskPolicyRepository
from .schemas import RiskPolicyCreate, RiskPolicyUpdate


class RiskPolicyNotFoundError(Exception):
    def __init__(self, policy_id: UUID):
        self.policy_id = policy_id
        super().__init__(f"RiskPolicy {policy_id} not found")
...
class RiskPolicyService:
    ...
    def __init__(self) -> None:
        self._repo = RiskPolicyRepository()
    ...
    def create_policy(self, db: Session, payload: RiskPolicyCreate) -> RiskPolicy:
    ...
    def get_policy(self, db: Session, policy_id: UUID) -> RiskPolicy:
        policy = self._repo.get_by_id(db, policy_id)
        if policy is None:
            raise RiskPolicyNotFoundError(policy_id)
        return policy

    def get_policy_by_portfolio(self, db: Session, portfolio_id: UUID) -> Optional[RiskPolicy]:
        return self._repo.get_by_portfolio(db, portfolio_id)

    def update_policy(self, db: Session, policy_id: UUID, payload: RiskPolicyUpdate) -> RiskPolicy:
