"""API interne — sync intents Ledgity depuis le portal Next.js."""
...
from .ledgity_intent_sync import (
    ensure_ledgity_intent_for_vault_transaction,
    mark_ledgity_intent_confirmed,
    mark_ledgity_intent_failed,
)

ledgity_intent_internal_router = APIRouter(
    prefix="/api/internal/transaction-intents/ledgity",
    tags=["transaction-intents-ledgity-internal"],
)


class LedgityVaultTxPayload(BaseModel):
...
class LedgityReceiptPayload(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
@ledgity_intent_internal_router.post("/pending")
def sync_pending(
    body: LedgityVaultTxPayload,
...
@ledgity_intent_internal_router.post("/receipt")
def sync_receipt(
    body: LedgityReceiptPayload,
    db: Session = Depends(get_db),
    _: None = Depends(_verify_internal_key),
):
    status_norm = body.vault_status.strip().lower()
    if status_norm == "success":
        result = mark_ledgity_intent_confirmed(
            db,
            person_id=body.person_id,
            vault_transaction_id=body.vault_transaction_id,
            tx_hash=body.tx_hash,
        ...
        result = mark_ledgity_intent_failed(
            db,
            person_id=body.person_id,
            vault_transaction_id=body.vault_transaction_id,
            tx_hash=body.tx_hash,
