...
@bootstrap_router.post("/bundle/{portfolio_id}/rebalancing/resume")
def mobile_bundle_rebalancing_resume(
    portfolio_id: str,
    db: Session = Depends(get_db),
    client: PeClient = Depends(mobile_app_client),
):
    """Reprise rééquilibrage — après signature LI.FI d'un leg (quote suivant ou terminal)."""
    from uuid import UUID as _UUID
    ...
    try:
        result = resume_rebalancing_portfolio(
            db, client_id=client.id, portfolio_id=pid, trigger="manual",
        )
        db.commit()
        return result
    except RebalancingPortfolioError as exc:
