...
def main() -> None:
    ...
    db = SessionLocal()
    try:
        items = _find_inflated_link_credits(db, person_id=person_id)
        print(f"Mode: {'DRY_RUN' if dry_run else 'APPLY'}")
        print(f"Inflated LINK swap credits found: {len(items)}")
        if not items:
        ...
        for item in items:
            ...
            print(
                f"  deposit={out['deposit_id']} person={out['person_id']} "
