...
export function BacktestsTab() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (strategyType === 'CPPI') {
        ...
        cppiParams = { ...CPPI_DEFAULTS, ...parsed }
        // Apply debug checkbox
        cppiParams.debug = cppiDebug
        // Remove undefined values
        const cppiMerged = cppiParams
        Object.keys(cppiMerged).forEach((key) => {
          if (cppiMerged[key] === undefined) {
            delete cppiMerged[key]
          }
        })
      }
      ...
      if (strategyType === 'CORE_SATELLITE') {
        ...
        // Merge with defaults (parsed can be partial or null)
        coreSatelliteParams = { ...CORE_SATELLITE_DEFAULTS, ...parsed }
        // Apply debug checkbox
        coreSatelliteParams.debug = coreSatelliteDebug
        // Remove undefined values
        const coreSatMerged = coreSatelliteParams
        Object.keys(coreSatMerged).forEach((key) => {
          if (coreSatMerged[key] === undefined) {
            delete coreSatMerged[key]
          }
        })
      }
  ...
  // Update selected instruments and strategy when bundle changes
  useEffect(() => {
    if (selectionType === 'bundle' && selectedBundleId) {
      const bundle = bundles.find(b => b.id === selectedBundleId)
      if (bundle) {
      ...
      // When switching to instruments, reset strategy to equal_weight if it was bundle_strategy
      setStrategyType(prev => prev === 'bundle_strategy' ? 'equal_weight' : prev)
    }
  }, [selectedBundleId, bundles, selectionType])

  return (
    <div>
      <h2 className="text-2xl font-semibold text-gray-900 mb-6">Backtests</h2>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {selectionType === 'bundle' ? (
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="bundle_strategy">Bundle Strategy (allocations du bundle)</SelectItem>
                    ...
                    <Select 
                      value={strategyType} 
                      onValueChange={(v: 'equal_weight' | 'momentum' | 'CPPI' | 'CORE_SATELLITE') => setStrategyType(v)}
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  {selectionType === 'bundle' && strategyType === 'bundle_strategy' && (
                    <p className="text-xs text-gray-500 mt-1">
                      La stratégie utilise les allocations définies dans le bundle (80% BTCUSD, 20% ETHUSD, etc.)
                    </p>
                  )}
                </div>
                ...
                {strategyType === 'CORE_SATELLITE' && (
                  <div className="space-y-3">
                    ...
                    <div className="text-xs text-gray-500">
                      <p className="mb-1">Key Parameters:</p>
                      <ul className="list-disc list-inside space-y-0.5">
                        <li><strong>allocation_mode</strong>: "te_target" | "utility_lambda" | "dynamic_cushion"</li>
                        <li><strong>target_te</strong> (0-1): Target tracking error</li>
                        <li><strong>core_yield</strong> (&gt;=0): Annual yield for core benchmark</li>
                        <li><strong>optimization_method</strong>: "grid" (V1) | "quadratic" (V2)</li>
                        <li><strong>shrinkage</strong>: Enable Ledoit-Wolf shrinkage (V2)</li>
                        <li><strong>lambda_risk</strong>: Risk aversion for utility_lambda mode (V2.1)</li>
                        <li><strong>multiplier</strong>: Multiplier for dynamic_cushion mode (V2.1)</li>
                        <li><strong>floor_rel_ratio</strong> (0-1): Relative floor for dynamic_cushion (V2.1)</li>
                      </ul>
                    </div>
                    <div className="flex items-center gap-2">
                      <input
                    ...
                    {selectionType === 'bundle' && (
                      <p className="text-xs text-gray-500 mt-1">
                        Bundle définit l'univers. Les poids sont optimisés (pas fixes). Benchmark est CORE.
                      </p>
                    )}
                  </div>
                )}
        ...
        <div className="space-y-6">
          {loadingResults ? (
            ...
            <>
              ...
              {(() => {
                const isCPPI = backtestDetail?.run?.strategy_type === 'CPPI'
                if (isCPPI && backtestSeries?.portfolio?.length > 0) {
                  // Diagnostic logs
                  console.log('[CPPI Charts] Rendering CPPI charts', {
                    strategy_type: backtestDetail.run.strategy_type,
                    portfolio_length: backtestSeries.portfolio.length,
                    first_bar_weights: backtestSeries.portfolio[0]?.weights_json,
                    strategy_params: backtestDetail.run.strategy_params_json,
                  })
                  return (
                    <Card>
                      <CardHeader>
                  ...
                  console.log('[CPPI Charts] CPPI detected but no portfolio data', {
                    strategy_type: backtestDetail.run.strategy_type,
                    has_portfolio: !!backtestSeries?.portfolio,
                    portfolio_length: backtestSeries?.portfolio?.length || 0,
                  })
                }
                return null
              })()}
              ...
              {backtestDetail.run.strategy_type === 'CORE_SATELLITE' && backtestSeries?.portfolio && backtestSeries.portfolio.length > 0 && (
                <Card>
                  <CardHeader>
