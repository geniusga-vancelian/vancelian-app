...
export async function POST(
  ...
  try {
    ...
    const res = await portalUpstreamFetch(
      `/api/app/bundle/${encodeURIComponent(portfolioId)}/rebalancing/preflight`,
      {
        method: 'POST',
    ...
    if (parseError) {
      console.error(
        '[api/portal/bundles/rebalancing/preflight POST] upstream parse error',
        parseError,
      )
    }
    return NextResponse.json(data, { status: res.status })
  } catch (error) {
    console.error('[api/portal/bundles/rebalancing/preflight POST]', error)
