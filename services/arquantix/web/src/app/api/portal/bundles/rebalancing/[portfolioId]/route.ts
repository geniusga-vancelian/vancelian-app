...
export async function POST(
  ...
  try {
    ...
    const res = await portalUpstreamFetch(
      `/api/app/bundle/${encodeURIComponent(portfolioId)}/rebalancing`,
      {
        method: 'POST',
    ...
    if (parseError) {
      console.error('[api/portal/bundles/rebalancing POST] upstream parse error', parseError)
    ...
    console.error('[api/portal/bundles/rebalancing POST]', error)
