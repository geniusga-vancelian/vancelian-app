...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = buildBackendUrl(`/api/app/alerts${qs ? `?${qs}` : ''}`)
    ...
    console.error('[api/mobile/flutter/alerts GET]', error)
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error('[api/mobile/flutter/alerts DELETE]', error)
    return NextResponse.json({ error: 'Internal server error', message: 'The request could not be completed.' }, { status: 500, headers: { 'Content-Type': 'application/json; charset=utf-8' } })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const url = buildBackendUrl('/api/app/alerts')
    const res = await fetch(url, {
      method: 'POST',
    ...
    console.error('[api/mobile/flutter/alerts POST]', error)
    return NextResponse.json({ error: 'Internal server error', message: 'The request could not be completed.' }, { status: 500, headers: { 'Content-Type': 'application/json; charset=utf-8' } })
  }
}
