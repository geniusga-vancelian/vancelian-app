...
export async function DELETE(
  ...
  try {
    ...
    const url = buildBackendUrl(`/api/app/orders/${orderId}`)
    const res = await fetch(url, {
      headers: upstreamHeadersWithAuth(_req), method: 'DELETE', signal: AbortSignal.timeout(5000) })
    if (res.status === 204) return new NextResponse(null, { status: 204 })
    const data = await res.json()
    return NextResponse.json(data, { status: res.status })
  } catch (error) {
    console.error('[api/mobile/flutter/orders/[orderId] DELETE]', error)
