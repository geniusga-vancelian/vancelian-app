  ...
  try {
    ...
    if (sessionFound && session) {
      try {
        ...
        console.error('[JWT Debug Proxy] JWT generation error:', jwtError.message)
        jwtGenerated = false
      }
    }
    
    console.log('[JWT Debug Proxy] Auth check:', {
    ...
    const backendUrl = buildBackendUrl('/api/diagnostics/jwt-debug')
    ...
    try {
      ...
      // Return debug info even if status is not 200 (because it's a debug endpoint)
      return NextResponse.json({
        ...backendBody,
        proxy_info: {
          session_found: sessionFound,
      ...
      console.error('[JWT Debug Proxy] Backend proxy error:', {
    ...
    console.error('[JWT Debug Proxy] Error:', error)
