...
export async function POST(request: NextRequest) {
  try {
    ...
    const { collectionId, sourceLocale, targetLocales, mode } = translateCollectionSchema.parse(body)
    ...
    for (const targetLocale of targetLocales) {
      try {
        const existing = await prisma.helpCollectionI18n.findUnique({
          where: {
            collectionId_locale: {
              collectionId,
              locale: targetLocale,
        ...
        if (existing && mode === 'missing') {
          ...
          await prisma.translationLog.create({
            data: {
              entityType: TranslationEntityType.HELP_COLLECTION,
        ...
        const i18n = await prisma.helpCollectionI18n.upsert({
          where: {
            collectionId_locale: {
              collectionId,
              locale: targetLocale,
        ...
        console.error(`Error translating collection to ${targetLocale}:`, error)
        results.errors.push({
          locale: targetLocale,
    ...
    console.error('Error translating collection:', error)
    return NextResponse.json(
