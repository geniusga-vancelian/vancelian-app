...
/**
 * Auto-traduction d'une AcademyCategory (clone de la route Help). Crée ou
 * met à jour les `AcademyCategoryI18n` pour les locales cibles avec
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const targetLocale of targetLocales) {
      try {
        const existing = await prisma.academyCategoryI18n.findUnique({
          where: {
            categoryId_locale: {
              categoryId,
              locale: targetLocale,
        ...
        if (existing && mode === 'missing') {
          ...
          await prisma.translationLog.create({
            data: {
              entityType: TranslationEntityType.ACADEMY_CATEGORY,
              entityId: categoryId,
              sourceLocale,
              targetLocale,
              mode,
        ...
        await prisma.academyCategoryI18n.upsert({
          where: {
            categoryId_locale: {
              categoryId,
              locale: targetLocale,
        ...
        console.error(`Error translating academy category to ${targetLocale}:`, err)
        results.errors.push({
          locale: targetLocale,
    ...
    console.error('Error translating academy category:', error)
