...
function getMetaFromData(data: unknown): { investmentTypeSlug: string | null; sortOrder: number } {
  ...
  return {
    investmentTypeSlug: typeof o.investmentTypeSlug === 'string' ? o.investmentTypeSlug : null,
    sortOrder: typeof o.sortOrder === 'number' ? o.sortOrder : 999,
  }
}

/** POST /api/admin/vaults/reorder — Déplace un vault (swap sortOrder avec le voisin). */
export async function POST(request: NextRequest) {
  try {
    ...
    const category = current.investmentTypeSlug ?? '__none__'
    const sameCategory = withMeta
      .map((w, i) => ({ ...w, index: i }))
      .filter((w) => (w.investmentTypeSlug ?? '__none__') === category)
      .sort((a, b) => a.sortOrder - b.sortOrder)

    const posInCategory = sameCategory.findIndex((s) => s.page.slug === slug)
    if (posInCategory < 0) return NextResponse.json({ error: 'Vault not in category' }, { status: 400 })
    ...
    // Nouvel ordre : on swap les positions dans le tableau
    ...
    // Réassigner sortOrder 0, 1, 2... pour éviter les doublons (ex. deux vaults à 0)
