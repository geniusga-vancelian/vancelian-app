...
describe('swapTokenApproval', () => {
  ...
  it('coalesces missing spender from LI.FI router address', () => {
    const payload = coalesceSwapTokenApproval(
      {
        required: true,
    ...
    assert.equal(isSwapTokenApprovalRequired(payload), true)
    assert.equal(payload.spender_address, '0x1231DEB6f5749EF6cE6943a275A1D3E7486F4EaE')
  })
