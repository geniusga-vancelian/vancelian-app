...
//
// Fork local : [ArticleDetailMarkdownBuilder] — ligne vide finale sous les tableaux.

// ignore_for_file: implementation_imports — kDefaultImageBuilder non exporté.
...
/// Espace entre la dernière ligne du tableau et la barre custom dessinée en
/// dessous (pour ne jamais recouvrir le contenu).
const double _kMarkdownTableScrollbarTopGap = 6;

/// Air vertical entre items d'une liste ordonnée (`ol`), sans augmenter le
/// [blockSpacing] global (souvent 0 pour les paragraphes du détail article).
const double _kOrderedListItemExtraSpacing = 8;
...
///
/// Fork local de [MarkdownBuilder] (flutter_markdown) pour le détail article :
/// scrollbar custom sous les tableaux ([_MarkdownTableHorizontalScrollbar]),
/// blocs de code rendus comme du paragraphe, espacement des items `ol`.
///
/// See also:
///
///  * [Markdown], which is a widget that parses and displays Markdown.
class ArticleDetailMarkdownBuilder implements md.NodeVisitor {
  /// Creates an object that builds a [Widget] tree from parsed Markdown.
  ArticleDetailMarkdownBuilder({
  ...
  void visitText(md.Text text) {
    ...
    if (_blocks.isNotEmpty && builders.containsKey(_blocks.last.tag)) {
      ...
      // Blocs ``` … ``` : rendu comme un paragraphe courant (style `p`), pas de
      // monospace / fond / scroll horizontal ni formatage « code » du delegate.
      final TextStyle preAsParagraph =
          styleSheet.p ?? _inlines.last.style ?? const TextStyle();
      child = _buildRichText(
        TextSpan(
          style: preAsParagraph,
          text: text.text,
        ),
        textAlign: _textAlignForBlockTag('p'),
      );
    } else {
      child = _buildRichText(
        TextSpan(
  ...
  void visitElementAfter(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      Widget child = tagBuilderWidget ?? defaultChild();
      ...
      if (_isListTag(tag)) {
        ...
        if (styleSheet.tableColumnWidth is FixedColumnWidth ||
          ...
          // Le [Scrollbar] Material peint son pouce sur la zone de son enfant
          // (= sur les dernières lignes du tableau). On le remplace par une
          // **vraie** barre dessinée dans un widget séparé sous la table : pas
          // d’overlay possible, fade-out auto quand le scroll s’arrête.
            ...
            builder: (BuildContext context,
              ...
              return Column(
                ...
                children: [
                  ...
                  const SizedBox(height: _kMarkdownTableScrollbarTopGap),
                  _MarkdownTableHorizontalScrollbar(
                    controller: tableScrollController,
                  ),
                ],
              );
  ...
  double _gapBeforeAppend(String? parentTag, String closedTag) {
    final double base = styleSheet.blockSpacing ?? 0;
    if (parentTag == 'ol' && closedTag == 'li') {
      return base + _kOrderedListItemExtraSpacing;
    }
    return base;
  }

  void _addBlockChild(Widget child, String closedTag) {
    final _BlockElement parent = _blocks.last;
    if (parent.children.isNotEmpty) {
      final gap = _gapBeforeAppend(parent.tag, closedTag);
      if (gap > 0) {
        parent.children.add(SizedBox(height: gap));
      }
    }
  ...
  void _addAnonymousBlockIfNeeded() {
    ...
    if (inline.children.isNotEmpty) {
      ...
      if (textPadding == EdgeInsets.zero) {
        _addBlockChild(wrap, _currentBlockTag ?? 'p');
      } else {
        final Padding padding = Padding(padding: textPadding, child: wrap);
        _addBlockChild(padding, _currentBlockTag ?? 'p');
      }
  ...
  WrapAlignment _wrapAlignmentForBlockTag(String? blockTag) {
    switch (blockTag) {
      ...
      case 'pre':
        // Aligné sur le corps de paragraphe (blocs ``` rendus comme du texte courant).
        return styleSheet.textAlign;
      case 'hr':
        break;
      case 'li':
...
/// Barre de défilement horizontale **dessinée sous** le tableau Markdown.
///
/// Remplace [Scrollbar] dont le pouce reste verrouillé au bord bas du viewport
/// (= sur les dernières lignes du tableau). Ici, on rend la barre comme un
/// widget séparé sous la table, donc **aucun overlay** sur le contenu.
///
/// Comportement :
/// - **Visible uniquement pendant le scroll** : fade-in au début du scroll,
///   fade-out après [_timeToFade] sans mouvement.
/// - **Pas d’interaction** (drag) : la barre n’est qu’un indicateur. Pour
///   scroller, on glisse directement sur la table (gesture habituel).
/// - **Auto-masquée** quand le tableau tient dans la largeur disponible
///   (`maxScrollExtent <= 0`).
class _MarkdownTableHorizontalScrollbar extends StatefulWidget {
  const _MarkdownTableHorizontalScrollbar({required this.controller});

  final ScrollController controller;

  @override
  State<_MarkdownTableHorizontalScrollbar> createState() =>
      _MarkdownTableHorizontalScrollbarState();
}

class _MarkdownTableHorizontalScrollbarState
    extends State<_MarkdownTableHorizontalScrollbar>
    with SingleTickerProviderStateMixin {
  static const double _thickness = 4;
  static const double _radius = 2;
  static const double _crossMargin = 2;
  static const double _minThumbWidth = 24;
  static const Duration _timeToFade = Duration(milliseconds: 700);
  static const Duration _fadeDuration = Duration(milliseconds: 250);
  static const Color _trackColor = Color(0x14000000);
  static const Color _thumbColor = Color(0x66000000);
  ...
  @override
  void didUpdateWidget(covariant _MarkdownTableHorizontalScrollbar oldWidget) {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      height: _thickness + 2 * _crossMargin,
        ...
        builder: (BuildContext context, Widget? _) {
          return LayoutBuilder(
            builder: (BuildContext context, BoxConstraints constraints) {
              ...
              final double total = maxScroll + viewport;
              final double trackWidth = constraints.maxWidth;
              final double thumbWidth = math.max(
                _minThumbWidth,
                (viewport / total) * trackWidth,
              );
              final double fraction =
                  (position.pixels / maxScroll).clamp(0.0, 1.0);
              final double thumbX = fraction * (trackWidth - thumbWidth);
