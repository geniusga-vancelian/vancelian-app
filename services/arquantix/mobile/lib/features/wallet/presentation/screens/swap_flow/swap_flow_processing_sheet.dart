...
class SwapFlowProcessingSheet extends StatefulWidget {
  const SwapFlowProcessingSheet({
    super.key,
  ...
  @override
  State<SwapFlowProcessingSheet> createState() =>
      _SwapFlowProcessingSheetState();
}

enum _SheetPhase { processing, success, error }

class _SwapFlowProcessingSheetState extends State<SwapFlowProcessingSheet> {
  ...
  Future<void> _execute() async {
    try {
      ...
      setState(() {
        _phase = _SheetPhase.error;
        _errorMessage = 'Erreur lors du swap';
      });
    }
  }
  ...
  Widget _buildSuccess() {
    final r = _result!;
    final toAmount = widget.formatCrypto(r.amountTo ?? 0);
    final fromAmount = widget.formatCrypto(r.amountFrom ?? widget.amountFrom);
    final feeText = r.feeInRefCurrency != null && r.feeInRefCurrency! > 0
        ? widget.fiatFormatter.format(r.feeInRefCurrency!)
        : null;
