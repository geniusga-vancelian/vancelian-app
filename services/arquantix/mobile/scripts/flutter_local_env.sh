...
if [[ "${PORTAL_PRIVY_OTP_DEV_MOCK_ENABLED:-}" == "true" ]]; then
  FLUTTER_EXTRA_DART_DEFINES+=(--dart-define=PORTAL_PRIVY_OTP_DEV_MOCK_ENABLED=true)
fi
if [[ -n "${PORTAL_PRIVY_OTP_DEV_FIXED_CODE:-}" ]]; then
  FLUTTER_EXTRA_DART_DEFINES+=(--dart-define=PORTAL_PRIVY_OTP_DEV_FIXED_CODE="$PORTAL_PRIVY_OTP_DEV_FIXED_CODE")
fi
