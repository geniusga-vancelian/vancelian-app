...
| **Date** | 2026-06-07 |
| **Statut** | Design actif — B1/B2/B2b/B3b/B3a/B3c mergés · **B4a en cours** (child factory only · PR · flag OFF · pas de runtime worker) |
...
CHILD_LEGS_CREATED
   ↓  (B4a child factory · 1 row child intent par leg du plan gelé)
EXECUTING
...
| `REBALANCE_PLAN_FROZEN` | planner step (B3b) | `plan_hash` + `planner_version` |
| `CHILD_LEGS_CREATED` | child factory (B4a) | child idempotency keys liés à `plan_hash` |
...
**Gate B4a** : pas de child intent sans `REBALANCE_PLAN_FROZEN` + `plan_hash` présent en metadata parent.
...
8. Créer child intents depuis le plan gelé (B4a) :
...
Parent FUNDING_REQUESTED
  ...
  → enqueue child leg creation depuis rebalance_plan (B4a factory · B4b runtime)
```
...
| **B3c** | `bundle_leg_settlement_handler.py` · `settle_bundle_leg_idempotently(child_intent_id)` · BUY USDC→AAVE Base · `child_report_hash` | Flag OFF (`BUNDLE_LEG_SETTLEMENT_HANDLER_ENABLED`) | **✅ Mergée** (PR `#57`) · deploy neutre TD `:154` · **test contrôlé prod GO** (`GO_BUNDLE_B3C_CONTROLLED_TEST_REPORT.md`) |
...
### Phase B4 — Child factory + runtime leg (découpage B4a / B4b)

> **Discipline** : B4a = factory pure (parent FROZEN → child #0) · B4b = runtime (child → swap → settlement B3c). Pas de WebApp · pas de Controller · pas de N legs avant B4 validé.

| PR | Scope | Runtime | Statut |
| --- | --- | --- | --- |
| **B4a** | `bundle_child_factory.py` · `create_bundle_child_intents_from_frozen_plan()` · 1 parent · 1 leg #0 BUY USDC→AAVE Base · `status=awaiting_swap` | ❌ Pure function · pas de worker/outbox | **🟡 PR ouverte** |
| **B4b** | Child runtime : child auto → quote/sign swap → settlement B3c · wiring outbox/worker | Flag OFF | ⏸ après B4a |
...
flowchart TD
  ...
  B3b --> B4a[B4a Child factory from frozen plan]
  B4a --> B4b[B4b Child runtime swap settle]
...
| 3 | Ordre B3/B4 ? | **B3b ✅** · B3a ✅ · **B3c ✅** (test contrôlé prod) · **B4a child factory** · B4b runtime · B3d PE writers |
...
1. **B4a merge** — `bundle_child_factory.py` · parent `REBALANCE_PLAN_FROZEN` → child #0 `awaiting_swap` · idempotent · pas de PE/CB/swap/settlement
2. **B4b** — pont child auto → swap frais → settlement B3c (worker/outbox · flag OFF)
