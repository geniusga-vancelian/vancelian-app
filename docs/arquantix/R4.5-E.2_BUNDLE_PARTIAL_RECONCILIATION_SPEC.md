...
## 14. E.2-C — Legacy cleanup ops (préparé, pas d’apply prod)

### Script ops (dry-run par défaut)

```bash
cd services/arquantix/api

# Audit read-only agrégé (read model + PE + ledger shadow)
...
# Dry-run (défaut) — affiche would_write, aucune mutation
python3 -m scripts.complete_legacy_bundle_with_cash_residual \
  --person-id 8b0e0044-f1ef-47a5-99d4-370598a77492 \
  --portfolio-id daea3720-e58e-410f-a796-3bbd541ac608 \
  --batch-id 8486fb48-09e6-421c-8654-8a0e5ad1b9be \
...
# python3 -m scripts.complete_legacy_bundle_with_cash_residual ... --apply
```
...
| Check | Règle |
|-------|--------|
| Read model | `status = reconciliation_required` |
| Intent | `intent_status = partial` |
| Lock | `zombie` ou lock actif stale (TTL) sur le batch |
| Progrès | ≥1 allocation confirmée **ou** `cash_residual_usdc > 0` |
| On-chain | Aucun swap `SUBMITTED` avec `tx_hash` |
| Ledger shadow | `verdict != DIFF` (batch scope) |
| Batch | `batch_id` = lock + intents + swaps |

### Would-write (apply)

| Cible | Action |
|-------|--------|
| `person_wallet_swaps` (pending sans tx) | `status → EXPIRED` (CBETH awaiting_signature stale) |
| `transaction_intents` | `metadata_json.terminal_outcome = completed_partial_allocation`, `legacy_cleanup`, `skipped_legs`, `cash_residual_usdc` ; legs pending → `failed` |
| `pe_portfolios.metadata` | `clear_invest_lock` (suppression `bundle_invest_lock`) |
| `pe_audit_events` | `ops.bundle_invest.completed_partial_allocation` + `idempotency_key` |
| **PE positions / cash / spot** | **Aucune mutation** |
| **Trading release** | **Interdit** |

### Tests

`pytest tests/test_bundle_legacy_cleanup.py -q` → 8 passed.

### Fichiers E.2-C

| Fichier | Rôle |
|---------|------|
| `bundle_legacy_cleanup.py` | Logique audit, validation, dry-run, apply idempotent |
| `scripts/complete_legacy_bundle_with_cash_residual.py` | CLI ops |
| `tests/test_bundle_legacy_cleanup.py` | 8 scénarios |

**Pas d’apply prod** dans ce livrable — revue humaine du dry-run requis avant `--apply`.
