...
|-------|-----|----------|---------------|
| **E.2-A** | Read model | API + type TS `BundleReconciliationState` ; agrégation PE/swaps/intent/lock ; tests unitaires | Non (lecture seule) — **implémenté** : `GET /api/app/bundle/reconciliation-state` · `GET /api/portal/bundles/reconciliation-state` |
