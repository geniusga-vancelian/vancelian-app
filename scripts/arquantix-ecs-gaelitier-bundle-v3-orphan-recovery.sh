#!/usr/bin/env bash
# Recovery bundle V3 orphelin — gaelitier Kings (audit puis apply).
#
#   ./scripts/arquantix-ecs-gaelitier-bundle-v3-orphan-recovery.sh
#   BUNDLE_V3_RECOVERY_APPLY=true ./scripts/arquantix-ecs-gaelitier-bundle-v3-orphan-recovery.sh
#
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
INLINE="${ROOT_DIR}/scripts/_gaelitier-bundle-v3-orphan-recovery-inline.py"
PAYLOAD="${ROOT_DIR}/scripts/arquantix-ecs-gaelitier-bundle-v3-orphan-recovery.payload.b64"
...
APPLY="${BUNDLE_V3_RECOVERY_APPLY:-false}"
...
S3_KEY="ops/ecs-payloads/gaelitier-bundle-v3-orphan-recovery/${PAYLOAD_SHA}.payload.b64"
...
OVERRIDES=$(RECOVERY_PAYLOAD_URL="$PAYLOAD_URL" CONTAINER_NAME="$CONTAINER_NAME" \
  BUNDLE_V3_RECOVERY_APPLY="$APPLY" python3 - <<'PY'
import json, os
bootstrap = (
    "import os,urllib.request,zlib,base64; "
    "u=os.environ['RECOVERY_PAYLOAD_URL']; "
...
print(json.dumps({"containerOverrides": [{
    ...
    "command": ["sh", "-c", cmd],
    "environment": [
        {"name": "RECOVERY_PAYLOAD_URL", "value": os.environ["RECOVERY_PAYLOAD_URL"]},
        {"name": "BUNDLE_V3_RECOVERY_APPLY", "value": os.environ.get("BUNDLE_V3_RECOVERY_APPLY", "false")},
    ],
}]}))
PY
)

echo "==> bundle V3 orphan recovery (apply=$APPLY)"
...
if [[ -n "$LOG_STREAM" && "$LOG_STREAM" != "None" ]]; then
  ...
  aws logs get-log-events --log-group-name "/ecs/$ECS_SERVICE" --log-stream-name "$LOG_STREAM" \
    --region "$AWS_REGION" --limit 200 --query 'events[*].message' --output text
fi
