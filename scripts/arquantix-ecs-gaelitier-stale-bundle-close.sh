...
#
#   ./scripts/arquantix-ecs-gaelitier-stale-bundle-close.sh
#   BUNDLE_STALE_CLOSE_APPLY=true ./scripts/arquantix-ecs-gaelitier-stale-bundle-close.sh
#
set -euo pipefail
ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
INLINE="${ROOT_DIR}/scripts/_gaelitier-stale-bundle-close-inline.py"
PAYLOAD="${ROOT_DIR}/scripts/arquantix-ecs-gaelitier-stale-bundle-close.payload.b64"
...
APPLY="${BUNDLE_STALE_CLOSE_APPLY:-false}"
...
S3_KEY="ops/ecs-payloads/gaelitier-stale-bundle-close/${PAYLOAD_SHA}.payload.b64"
...
OVERRIDES=$(RECOVERY_PAYLOAD_URL="$PAYLOAD_URL" CONTAINER_NAME="$CONTAINER_NAME" \
  BUNDLE_STALE_CLOSE_APPLY="$APPLY" python3 - <<'PY'
import json, os
bootstrap = (
...
print(json.dumps({"containerOverrides": [{
    ...
    "environment": [
        ...
        {"name": "BUNDLE_STALE_CLOSE_APPLY", "value": os.environ.get("BUNDLE_STALE_CLOSE_APPLY", "false")},
    ],
}]}))
PY
)

echo "==> stale bundle close (apply=$APPLY)"
