...
PAYLOAD="${ROOT_DIR}/scripts/arquantix-ecs-bundle-b5-controlled-test.payload.b64"
MODE="${1:-}"
...
if [[ -z "$MODE" ]]; then
  echo "Usage: $0 <baseline|reconcile_parent|audit>" >&2
  exit 1
fi
shift || true
...
S3_KEY="ops/ecs-payloads/bundle-b5-controlled-test/${PAYLOAD_SHA}.payload.b64"
...
OVERRIDES=$(BUNDLE_B5_TEST_MODE="$MODE" \
  BUNDLE_B5_TEST_CONFIRM="${BUNDLE_B5_TEST_CONFIRM:-}" \
  PARENT_INTENT_ID="${PARENT_INTENT_ID:-}" \
  BUNDLE_B5_PAYLOAD_URL="$PAYLOAD_URL" \
  CONTAINER_NAME="$CONTAINER_NAME" \
  python3 - <<'PY'
import json, os

env = [{"name": "BUNDLE_B5_PAYLOAD_URL", "value": os.environ["BUNDLE_B5_PAYLOAD_URL"]}]
for key in ("BUNDLE_B5_TEST_MODE", "BUNDLE_B5_TEST_CONFIRM", "PARENT_INTENT_ID"):
    val = os.environ.get(key, "")
    if val:
        env.append({"name": key, "value": val})

if os.environ.get("BUNDLE_B5_TEST_MODE") == "reconcile_parent":
    env.append({"name": "BUNDLE_PARENT_CONTROLLER_ENABLED", "value": "true"})

bootstrap = (
    "import os,urllib.request,zlib,base64; "
    "u=os.environ['BUNDLE_B5_PAYLOAD_URL']; "
...
echo "==> B5 controlled test mode=$MODE"
...
aws logs get-log-events \
  ...
  | python3 -c "import sys,re,json; t=sys.stdin.read(); m=re.search(r'\{.*\"phase\"\s*:\s*\"bundle_b5_controlled_test\".*\}', t, re.S); print(json.dumps(json.loads(m.group()), indent=2) if m else 'JSON_NOT_FOUND\n'+t[-2000:])" \
  || true
