...
if [[ -z "${BASE_RPC_URL_PRIMARY:-}" ]]; then
  echo "ERREUR: BASE_RPC_URL_PRIMARY requis (env, .env.arquantix ou $LOCAL_RPC_FILE)." >&2
  echo "Exemple: BASE_RPC_URL_PRIMARY='https://base-mainnet.g.alchemy.com/v2/...' $0" >&2
  exit 1
fi
...
MORPHO_USDC_BETA_MIN_DEPOSIT_USDC="${MORPHO_USDC_BETA_MIN_DEPOSIT_USDC:-10}"
MORPHO_USDC_BETA_MAX_DEPOSIT_USDC="${MORPHO_USDC_BETA_MAX_DEPOSIT_USDC:-100}"
MORPHO_USDC_BETA_MAX_USER_EXPOSURE_USDC="${MORPHO_USDC_BETA_MAX_USER_EXPOSURE_USDC:-500}"
MORPHO_USDC_BETA_MAX_GLOBAL_EXPOSURE_USDC="${MORPHO_USDC_BETA_MAX_GLOBAL_EXPOSURE_USDC:-5000}"
...
if [[ "$MORPHO_USDC_BETA_ENABLED" == "true" && -z "$MORPHO_USDC_BETA_PERSON_IDS" && -z "$MORPHO_USDC_BETA_EMAILS" && -z "$MORPHO_USDC_BETA_PROFILE_TAG" && "$MORPHO_USDC_BETA_INCLUDE_ADMINS" != "true" ]]; then
  echo "ERREUR: MORPHO_USDC_BETA_ENABLED=true sans allowlist (PERSON_IDS, EMAILS, PROFILE_TAG ou INCLUDE_ADMINS)." >&2
  exit 1
fi

if [[ "$MORPHO_USDC_BETA_ENABLED" == "true" && -z "$BASE_RPC_PRIMARY_ARN" && -z "${BASE_RPC_URL_PRIMARY:-}" ]]; then
...
morpho_env = {
    ...
    "MORPHO_USDC_BETA_PERSON_IDS": sys.argv[9],
    "MORPHO_USDC_BETA_EMAILS": sys.argv[10],
    "MORPHO_USDC_BETA_PROFILE_TAG": sys.argv[11],
    "MORPHO_USDC_BETA_INCLUDE_ADMINS": sys.argv[12],
    "MORPHO_USDC_BETA_MIN_DEPOSIT_USDC": sys.argv[13],
    "MORPHO_USDC_BETA_MAX_DEPOSIT_USDC": sys.argv[14],
    "MORPHO_USDC_BETA_MAX_USER_EXPOSURE_USDC": sys.argv[15],
    "MORPHO_USDC_BETA_MAX_GLOBAL_EXPOSURE_USDC": sys.argv[16],
    "MORPHO_RECONCILIATION_TOLERANCE_RAW": sys.argv[17],
    "MORPHO_ALERT_MISMATCH_TOLERANCE_RAW": sys.argv[18],
    "MORPHO_PENDING_ALERT_MINUTES": sys.argv[19],
