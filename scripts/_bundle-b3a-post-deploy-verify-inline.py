"""Post-deploy neutre B3a — bundle funding handler · flag OFF · aucun runtime."""
...
def main() -> None:
    ...
    try:
        ...
        funding_receipt_parents = db.execute(
            text(
                """
                SELECT COUNT(*)
                FROM transaction_intents
                WHERE product_type = 'bundle_invest'
                  AND (
                    ...
                    OR COALESCE(metadata_json->'bundle_funding'->>'funding_settled', '') = 'true'
                  )
                """
            )
        ).scalar()
        ...
        flag_env = os.environ.get("BUNDLE_FUNDING_HANDLER_ENABLED", "").strip().lower()
        ...
        worker_src = ""
        try:
            from services.portfolio_engine.bundles.event_driven.bundle_funding_handler import (  # noqa: F401
            ...
            import services.portfolio_engine.bundles.event_driven.bundle_funding_handler as bfh

            module_imported = True
            handler_callable = callable(settle_bundle_funding_idempotently)
            flag_off_runtime = not bundle_funding_handler_enabled()
            module_src = inspect.getsource(bfh)
        ...
        runtime_wired = (
            "settle_bundle_funding_idempotently" in orchestrator_src
            or "bundle_funding_handler" in orchestrator_src
            or "settle_bundle_funding_idempotently" in worker_src
        )
        ...
        result = {
            ...
            "flags": {
                "BUNDLE_FUNDING_HANDLER_ENABLED": flag_env or None,
                "bundle_funding_handler_enabled()": flag_off_runtime,
            },
            "neutralite": {
                "bundle_scope_bundle_invest_locks": bundle_scope_locks,
                "bundle_leg_or_child_intents": child_intents,
                "bundle_funding_receipt_or_settled_parents": funding_receipt_parents,
                "pe_atoms": pe,
            ...
            "runtime_wiring": {
                "bundle_funding_handler_module_present": module_imported,
                "settle_bundle_funding_callable": handler_callable,
                ...
                "orchestrator_or_worker_calls_funding_handler": runtime_wired,
            },
            "all_checks_pass": (
                health_ok
                and str(alembic) == "176"
